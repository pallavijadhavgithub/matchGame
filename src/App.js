...
class App extends Component {
  state = {
    score: 0,
    activeTabId: tabsList.tabId[0],
  }

  displayFruitsList = tabId => {
    // console.log(tabId)
    const filteredTab = imagesList.filter(eachImage =>
  ...
  render() {
    ...
    const thumbnail = Math.floor(Math.random() * 10)

    const filteredTab = this.displayFruitsList(activeTabId)
    ...
    return (
      <div className="container">
        <div className="navbar-container">
          ...
          <div className="score-container">
            ...
            <img
              ...
              className="timer-img"
            />
            <p className="span">60 sec</p>
          </div>
        </div>
        <div className="second-container">
          <img
            src={imagesList[thumbnail].imageUrl}
            className="images"
            alt="imageUrl"
          />
        </div>
        ...
        <ul>
          {filteredTab.map(eachList => (
            <ImagesTab eachList={eachList} />
          ))}
        </ul>
      </div>
    )
